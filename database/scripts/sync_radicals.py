#!/usr/bin/env python3
"""
Radicals Reference Data Sync Script

Downloads the radicals.txt reference file from mifunetoshiro/kanjium repository.
This provides additional metadata for radicals (meanings, readings, notes etc.).
"""
...
# Configuration
REPO_OWNER = "mifunetoshiro"
REPO_NAME = "kanjium"
BRANCH = "master"
FILE_PATH = "data/source_files/radicals.txt"
...
STATE_FILE = Path(os.getenv("RADICALS_REF_STATE_FILE", BASE_DIR / "database" / "scripts" / ".radicals_ref_commit"))
TARGET_FILE = SOURCE_DIR / "radfile" / "reference.txt"
...
def download_radicals_txt():
    """Downloads the radicals.txt file from GitHub."""
...
def main():
    print("=" * 60)
    print("Radicals Reference Data Sync Script")
    print("=" * 60)
    ...
    if local_sha == latest_sha and TARGET_FILE.exists():
        print(f"Radicals reference data is up to date (SHA: {local_sha[:7]}).")
        return 0
